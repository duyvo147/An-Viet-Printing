...
const OrderList = ({ orders, onEdit, onDelete, title = 'Quản lý đơn hàng', userRole, users = [], onPrint }: { orders: Order[], onEdit: (o: Order) => void, onDelete?: (id: string) => void, title?: string, userRole?: string, users?: UserProfile[], onPrint: (order: Order, type: 'quote' | 'delivery' | 'payment_request') => void }) => {
  ...
  const paymentFilterRaw = searchParams.get('payment') || '';
  const selectedPayments = paymentFilterRaw ? paymentFilterRaw.split(',') : [];
  
  const creatorFilterRaw = searchParams.get('creator') || '';
  const selectedCreators = creatorFilterRaw ? creatorFilterRaw.split(',') : [];
  ...
  const PAYMENT_OPTIONS: { id: PaymentStatus, label: string, color: string }[] = [
    { id: 'unpaid', label: 'Chưa trả', color: 'bg-rose-500' },
    { id: 'partial', label: 'Cọc/Một phần', color: 'bg-amber-500' },
    { id: 'paid', label: 'Đã trả', color: 'bg-emerald-500' },
  ];
  ...
  const togglePayment = (payment: string) => {
    ...
    if (selectedPayments.includes(payment)) {
      newSelected = selectedPayments.filter(p => p !== payment);
    } else {
      newSelected = [...selectedPayments, payment];
    }
    updateParams({ payment: newSelected.length > 0 ? newSelected.join(',') : null });
  };

  const toggleCreator = (uid: string) => {
    let newSelected: string[];
    if (selectedCreators.includes(uid)) {
      newSelected = selectedCreators.filter(c => c !== uid);
    } else {
      newSelected = [...selectedCreators, uid];
    }
    updateParams({ creator: newSelected.length > 0 ? newSelected.join(',') : null });
  };
  ...
  const filteredOrders = orders.filter(o => {
    ...
    const matchesPayment = selectedPayments.length === 0 || selectedPayments.includes(o.paymentStatus);
    const matchesCreator = selectedCreators.length === 0 || selectedCreators.includes(o.createdBy);
  ...
  const exportOrders = () => {
    const wsData = filteredOrders.map(o => ({
      ...
      'Ngày Tạo': format(o.createdAt.toDate(), 'dd/MM/yyyy HH:mm'),
      'Khách Hàng': o.customerName,
      'SĐT': o.customerPhone,
      'Mã VAT': o.vatInvoiceCode || '',
      'Số Sản Phẩm': o.items.length,
      'Tổng Tiền': o.totalAmount,
      'Đã Trả': o.paidAmount,
      'Còn Nợ': o.debtAmount,
      'Trạng Thái': o.status,
      'Người Tạo': getCreatorName(o.createdBy)
    }));

    const ws = XLSX.utils.json_to_sheet(wsData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Danh sách đơn hàng");
    XLSX.writeFile(wb, `Danh_sach_don_hang_${format(new Date(), 'ddMMyyyy_HHmm')}.xlsx`);
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        {userRole !== 'production' && (
          <Link to="/orders/new" className="bg-indigo-600 text-white px-4 py-2 rounded-xl flex items-center gap-2 hover:bg-indigo-700 transition-all shadow-md shadow-indigo-100 active:scale-95">
